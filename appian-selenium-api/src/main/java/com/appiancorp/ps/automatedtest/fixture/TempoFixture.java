...
import com.appiancorp.ps.automatedtest.tempo.interfaces.TempoAcceptButton;
import com.appiancorp.ps.automatedtest.tempo.interfaces.TempoBox;
import com.appiancorp.ps.automatedtest.tempo.interfaces.TempoButton;
import com.appiancorp.ps.automatedtest.tempo.interfaces.TempoCard;
import com.appiancorp.ps.automatedtest.tempo.interfaces.TempoCardChoice;
import com.appiancorp.ps.automatedtest.tempo.interfaces.TempoCardChoiceField;
...
import com.appiancorp.ps.automatedtest.tempo.interfaces.TempoClickableChart;
import com.appiancorp.ps.automatedtest.tempo.interfaces.TempoConfirmationDialog;
import com.appiancorp.ps.automatedtest.tempo.interfaces.TempoDocumentImageLink;
import com.appiancorp.ps.automatedtest.tempo.interfaces.TempoDropdownSearchBox;
...
import com.appiancorp.ps.automatedtest.tempo.interfaces.TempoIconLink;
import com.appiancorp.ps.automatedtest.tempo.interfaces.TempoLink;
import com.appiancorp.ps.automatedtest.tempo.interfaces.TempoLinkUrl;
import com.appiancorp.ps.automatedtest.tempo.interfaces.TempoMenuWidget;
...
import com.appiancorp.ps.automatedtest.tempo.interfaces.TempoRecordActionField;
...
import com.appiancorp.ps.automatedtest.tempo.interfaces.TempoSignatureField;
import com.appiancorp.ps.automatedtest.tempo.interfaces.TempoStampField;
import com.appiancorp.ps.automatedtest.tempo.interfaces.TempoTagField;
import com.appiancorp.ps.automatedtest.tempo.interfaces.TempoVideo;
import com.appiancorp.ps.automatedtest.tempo.interfaces.TempoWebContent;
import com.appiancorp.ps.automatedtest.tempo.interfaces.TempoGeneralizedText;
...
public class TempoFixture extends BaseFixture {
  ...
  public boolean verifyNewsFeedContainingTextIsNotPresent(String newsText) {
    return TempoNewsItem.getInstance(settings).waitForReturn(false, settings.getNotPresentTimeoutSeconds(), newsText);
  }
   ...
   *
   * @param message
   *          Message to include in Kudos
   * @param recipient
   *          User to send Kudos to
   */
   ...
   *
   * @param lockStatus
   *          String representing whether the message should be locked or unlocked
   * @param message
   *          Body of message to send
   * @param recipients
   *          List of users/groups to send message to
   */
   ...
   * @param message
   *          Description of task to send
   * @param recipient
   *          User to send task to
   */
   ...
   * @param message
   *          Message to include in post body
   * @param participants
   *          List of participants to send post to
   */
   ...
   * @param newsText
   *          Text of news post to find and star
   */
   ...
   * @param newsText
   *          Text of news post to verify is starred
   ...
   * @param comment
   *          Comment to add to news post
   * @param newsText
   *          Text of news post to add comment to
   */
   ...
   * @param filterName
   *          Name of predefined filter for news feed (e.g. Participating, Kudos, etc.)
   */
   ...
   * @param newsText
   *          Text of news post to click link for
   ...
   * @param newsText
   *          Text of news post used to associate user profile circle
   ...
   * @param newsText
   *          Text of news post used to associate user profile link
   ...
   * @param taskText
   *          Text to search for in the task feed
  ...
  public boolean verifyTaskFeedContainingTextIsNotPresent(String taskText) {
    return TempoSocialTaskItem.getInstance(settings).waitForReturn(false, settings.getNotPresentTimeoutSeconds(), taskText);
  }
   ...
   * @param taskText
   *          Text of social task to close
   * @param comment
   *          Comment to add before closing social task
   */
   ...
   *
   * @param userFilter
   *          User Filter to populate (partial names are acceptable)
   * @param value
   *          Value to select within User Filter dropdown
   */
   ...
   *          User Filter to populate (partial names are acceptable)
   * @param startDate
   *          Start date to enter in the date range (format mm/dd/yyyy)
   * @param endDate
   *          End date to enter in the date range (format mm/dd/yyyy)
   */
  ...
  public boolean verifyRecordIsNotPresent(String recordName) {
    return TempoRecord.getInstance(settings).waitForReturn(false, settings.getNotPresentTimeoutSeconds(), recordName);
  }
  ...
  public boolean verifyRecordRelatedActionIsNotPresent(String relatedActionName) {
    return TempoRecord.getInstance(settings).waitForReturn(false, settings.getNotPresentTimeoutSeconds(), relatedActionName);
  }
   ...
   *
   * @param headers
   *          One or more headers, starting with the first column, which uniquely identify the target grid to be checked for matching data.
  ...
  public boolean verifyActionIsNotPresent(String actionName) {
    return TempoAction.getInstance(settings).waitForReturn(false, settings.getNotPresentTimeoutSeconds(), actionName);
  }
  ...
  public boolean verifyApplicationFilterIsNotPresent(String applicationName) {
    return TempoAction.getInstance(settings).waitForReturn(false, settings.getNotPresentTimeoutSeconds(), applicationName);
  }
   ...
   *
   * @param fieldType
   *          Can only currently accept TEXT, PARAGRAPH, FILE_UPLOAD, MILESTONE and PICKER
   * @param fieldName
   *          Can either be the label of the field or a label with an index
   * @param fieldValues
   *          An array of strings containing the values to populate into the interface
  ...
  public void populateFieldWithPlaceholderWith(String placeholder, String[] fieldValues) {
    ...
    TempoFieldFactory.getInstance(settings).populateMultiple(fieldValues, false, Constants.FIELD_LOCATOR_PLACEHOLDER + placeholder);
  }
  ...
  public void populateFieldWithInstructionsWith(String instructions, String[] fieldValues) {
    ...
    TempoFieldFactory.getInstance(settings).populateMultiple(fieldValues, false, Constants.FIELD_LOCATOR_INSTRUCTIONS + instructions);
  }
  ...
  public void populateFieldWithTooltipWith(String tooltip, String[] fieldValues) {
    ...
    TempoFieldFactory.getInstance(settings).populateMultiple(fieldValues, false, Constants.FIELD_LOCATOR_TOOLTIP + tooltip);
  }
   ...
   *
   * @param fieldName
   *          Can either be the label of the dropdown or a label with an index
   * @param searchValue
   *          A string to populate the search box with and select from the dropdown
   */
   ...
   *
   * @param tagField
   *            Name or Name[Index] of tag field
   ...
   *
   * @param tagItem
   *            Name or Name[Index] of tag item
   ...
   *
   * @param gridName
   *          Name of the grid using label or index
   * @return JSON object of label:values
   */
   ...
   *
   * @param fieldName
   *          Can either be a name or or field index or a name and index, e.g. 'Text Field' or 'Text Field[1]'
  ...
  public boolean verifyFieldWithPlaceholderContains(String placeholder, String[] fieldValues) {
    TempoField.getInstance(settings).waitFor(Constants.FIELD_LOCATOR_PLACEHOLDER + placeholder);
    return TempoFieldFactory.getInstance(settings).containsMultiple(fieldValues, Constants.FIELD_LOCATOR_PLACEHOLDER + placeholder);
  }
  ...
  public boolean verifyFieldWithInstructionsContains(String instructions, String[] fieldValues) {
    TempoField.getInstance(settings).waitFor(Constants.FIELD_LOCATOR_INSTRUCTIONS + instructions);
    return TempoFieldFactory.getInstance(settings).containsMultiple(fieldValues, Constants.FIELD_LOCATOR_INSTRUCTIONS + instructions);
  }
  ...
  public boolean verifyFieldWithTooltipContains(String tooltip, String[] fieldValues) {
    TempoField.getInstance(settings).waitFor(Constants.FIELD_LOCATOR_TOOLTIP + tooltip);
    return TempoFieldFactory.getInstance(settings).containsMultiple(fieldValues, Constants.FIELD_LOCATOR_TOOLTIP + tooltip);
  }
   ...
   *          Values to populate
   */
  public void populateGridColumnRowWithPartiallyMatchingPickerFieldSuggestion(String gridName, String columnName, String rowNum, String[] fieldValues) {
    TempoGridCell.getInstance(settings).waitFor(gridName, columnName, rowNum);
   ...
   *
   * @param linkName
   *          Name of link to verify is not present
   ...
   *
   * @param iconAltTextName
   *          alt text of icon to click
   */
   ...
   *
   * @param documentImageLinkAltTextName
   *          alt text of document image to link
   */
   ...
   * @param chartLabel
   *  the label, including the index if necessary, of the chart to download
   */
   ...
   * @param barChartLabel
   *    The index of the bar chart out of all bar charts on the page
   * @param barNumber
   *    The index of a bar in the bar chart.
   ...
   * @param columnChartLabel
   *    The index of the column chart out of all column charts on the page
   * @param columnNumber
   *    The index of a column in the column chart.
   *    Corresponds to the order of data in the chartSeries in its definition.
   */
   ...
   * @param pieChartLabel
   *    The index of the pie chart out of all pie charts on the page
   * @param pieSliceNumber
   *    The index of the slice in the pie chart.
   ...
   * @param lineChartLabel
   *    The index of the line chart out of all line charts on the page
   * @param pointNumber
   *    The index of the point from going from left to right in the line chart.
   *    Corresponds to the order of data in the chartSeries in its definition.
   */
   ...
   * @param indexOfField
   *    The index of the record action field out of all record action fields on the page
   * @param action
   *    The action from the dropdown that the user wants to click on
   */
  ...
  public boolean verifyButtonIsNotPresent(String buttonName) {
    return TempoButton.getInstance(settings).waitForReturn(false, settings.getNotPresentTimeoutSeconds(), buttonName);
  }
  ...
  */
   ...
   *
   * @param gaugeField
   *            Name or Name[Index] of gauge field
   * @param percentage
   *            Value of percentage
   ...
   *
   * @param stampField
   *            Name or Name[Index] of stamp field
   ...
   *            Name or Name[Index] of stamp field
   * @param text
   *            Text expected in the stamp field
  ...
  public boolean verifyChartIsNotPresent(String chartLabel) {
    return TempoChart.getInstance(settings).waitForReturn(false, settings.getNotPresentTimeoutSeconds(), chartLabel);
  }
   ...
   *
   * @param sectionLabel
   *          Text to search for section label
  ...
  public boolean verifySectionIsNotPresent(String sectionLabel) {
    return TempoSection.getInstance(settings).waitForReturn(false, settings.getNotPresentTimeoutSeconds(), sectionLabel);
  }
   ...
   *
   * @param cardLabel
   *          Text to search for card accessibility text
   ...
   *
   * @param cardName
   *          Name of the card choice
   * @param fieldName
   *          Name of the card choice field the card is in
   */
  ...
  public boolean verifyVideoIsNotPresent(String videoSource) {
    return TempoVideo.getInstance(settings).waitForReturn(false, settings.getNotPresentTimeoutSeconds(), videoSource);
  }
  ...
  public boolean verifyWebContentIsNotPresent(String webContentSource) {
    return TempoWebContent.getInstance(settings).waitForReturn(false, settings.getNotPresentTimeoutSeconds(), webContentSource);
  }
   ...
   *
   * @param textOnInterface
   *          text user is seeking to verify
   *
  ...
  public boolean verifyTextIsNotPresent(String textOnInterface) {
    return TempoAssertion.getInstance(settings).waitForReturn(false, settings.getNotPresentTimeoutSeconds(), textOnInterface);
  }
}
