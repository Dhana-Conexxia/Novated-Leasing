...
public class TempoGrid extends AbstractTempoField implements WaitFor, Container, ContainerCaptureable {
  ...
  public String getXpath(String... params) {
    ...
    if (isFieldIndex(gridName)) {
      ...
      (new WebDriverWait(settings.getDriver(), Duration.ofSeconds(settings.getTimeoutSeconds()))).until(ExpectedConditions.visibilityOfElementLocated(By.xpath(xpathFormat(XPATH_ABSOLUTE_GRID_BY_LABEL, gridName))));
  ...
  private JSONArray getGridContents(WebElement grid) {
    List<WebElement> headers = grid.findElement(By.xpath(XPATH_RELATIVE_HEADER_ROW)).findElements(By.xpath(XPATH_RELATIVE_COLUMN));
    ...
    for (int i = 1; i <= headers.size(); i++) {
      ...
      try {
        WebElement accessibility = ((RemoteWebElement)headers.get(i-1)).findElement(By.className("GridHeaderCell---accessibilityhidden"));
    ...
    for(int i = 1; i <= rows.size(); i++) {
      ...
      for (int j = 1; j <= columns.size(); j++) {
        WebElement cell = TempoGridCell.getInstance(settings).getGridWebElement(grid, null, "[" + j + "]", "[" + i + "]");
        try {
  ...
  private int countLabels(String search, List<String> labels) {
    ...
    for (String label: labels) {
      if (label.startsWith(search)) count +=1 ;
    }
