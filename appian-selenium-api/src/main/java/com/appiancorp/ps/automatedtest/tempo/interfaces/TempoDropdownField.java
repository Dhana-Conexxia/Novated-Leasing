...
public class TempoDropdownField extends AbstractTempoField {
  ...
  private static final String XPATH_RELATIVE_DROPDOWN_FIELD_INPUT = Settings.getByConstant("xpathRelativeDropdownFieldInput");
  private static final String XPATH_RELATIVE_DROPDOWN_FIELD_INPUT_HIDDEN = Settings.getByConstant("xpathRelativeDropdownFieldInputHidden");
  ...
  private static final String XPATH_DROPDOWN_OPTION_ACTIVE = Settings.getByConstant("xpathDropdownOptionActive");
    ...
    if (!isMultipleDropdown) {
      ...
      while (!reachedBottom) {
        for (WebElement selectedItem : selectedItems) {
          boolean isSelected = Boolean.parseBoolean(selectedItem.getAttribute(XPATH_DROPDOWN_OPTION_SELECTED));
          boolean isChecked = atMostVersion(23.3) && Boolean.parseBoolean(selectedItem.getAttribute(XPATH_DROPDOWN_OPTION_CHECKED));
  ...
  public void clear(WebElement fieldLayout, String... params) {
    if (LOG.isDebugEnabled()) LOG.debug("CLEAR [" + fieldLayout.getText() + "]");
    ...
    if(!isMultipleDropdown(selectField)) {
      ...
      while (!reachedBottom) {
        for (Iterator<WebElement> iterator = listItems.iterator(); iterator.hasNext(); ) {
          ...
          boolean isChecked = atMostVersion(23.3) && Boolean.parseBoolean(next.getAttribute(XPATH_DROPDOWN_OPTION_CHECKED));
          if (isSelected || isChecked) {
            WebDriverWait webDriverWait = new WebDriverWait(settings.getDriver(), Duration.ofSeconds(settings.getTimeoutSeconds()));
