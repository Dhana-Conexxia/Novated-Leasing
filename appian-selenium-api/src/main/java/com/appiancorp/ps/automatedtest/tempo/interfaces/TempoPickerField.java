...
public class TempoPickerField extends AbstractTempoField {
  ...
  protected static final String XPATH_ABSOLUTE_PICKER_SUGGESTION_CONTAINS = Settings.getByConstant("xpathAbsolutePickerSuggestionContains");
  ...
  private static final String XPATH_ABSOLUTE_PICKER_SUGGESTION_BOX = Settings.getByConstant("xpathAbsolutePickerSuggestionBox");
  ...
  public void populate(WebElement fieldLayout, String... params) {
    ...
    new WebDriverWait(settings.getDriver(), Duration.ofSeconds(settings.getTimeoutSeconds())).until(ExpectedConditions.presenceOfElementLocated(By.xpath(XPATH_ABSOLUTE_PICKER_SUGGESTION_BOX)));
  ...
  public void clear(WebElement fieldLayout, String... params) {
    while(true) {
      // We can not load all of the choices and then loop through them to click, because once 1st item is clicked, the DOM structure
