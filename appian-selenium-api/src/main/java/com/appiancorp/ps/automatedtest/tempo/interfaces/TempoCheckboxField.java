...
public class TempoCheckboxField extends AbstractTempoField {
  ...
  public String getXpathForCheckbox(boolean isContains, String... params) {
    ...
    if (isContains && Settings.getVersion().compareTo(new Version(17, 1)) >= 0) {
      fixFor17_1 = "//input";
    }

    if (isFieldIndex(fieldValue)) {
      int index = getIndexFromFieldIndex(fieldValue);
      return xpathFormat(XPATH_RELATIVE_CHECKBOX_FIELD_CHOICE_INDEX + fixFor17_1, index);
    } else {
      return xpathFormat(XPATH_RELATIVE_CHECKBOX_FIELD_CHOICE_LABEL + fixFor17_1, fieldValue);
    }
  }
