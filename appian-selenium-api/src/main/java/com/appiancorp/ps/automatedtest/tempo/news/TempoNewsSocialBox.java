...
public class TempoNewsSocialBox extends AppianObject implements Refreshable, Clickable {
  ...
  public boolean waitForReturn(boolean waitForPresent, int timeout, String... params) {
    ...
    try {
      if (waitForPresent) {
        (new WebDriverWait(settings.getDriver(), Duration.ofSeconds(timeout))).until(ExpectedConditions.elementToBeClickable(By.xpath(xpath)));
      } else {
        (new WebDriverWait(settings.getDriver(), Duration.ofSeconds(timeout))).until(ExpectedConditions.invisibilityOfElementLocated(By.xpath(xpath)));
      }
      return true;
