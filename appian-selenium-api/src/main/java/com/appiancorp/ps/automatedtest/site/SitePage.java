...
public class SitePage extends AppianObject implements WaitFor, Clickable, Navigateable {
  ...
  private static final String XPATH_RELATIVE_SITE_PAGE_LINK_IN_SITE_GROUP = Settings.getByConstant("xpathRelativeSitePageInGroup");
  private static final String XPATH_ABSOLUTE_SITE_LOGOUT_LINK = Settings.getByConstant("xpathAbsoluteSiteLogoutLink");
  private static final String XPATH_ABSOLUTE_SITE_USER_PROFILE_LINK = Settings.getByConstant("xpathAbsoluteSiteUserProfileLink");
  private static final String XPATH_ABSOLUTE_SITE_PROFILE_MENU_LINK = Settings.getByConstant("xpathAbsoluteSiteProfileMenuLink");
  private static final String XPATH_ABSOLUTE_SITE_SETTINGS_MENU_LINK = Settings.getByConstant("xpathAbsoluteSiteSettingsMenuLink");
  private static final String XPATH_ABSOLUTE_SITES_DISCOVERABILITY_MENU_LINK = Settings.getByConstant("xpathSitesDiscoverabilityMenu");
  private static final String XPATH_RELATIVE_SITENAME_MENU_LINK = Settings.getByConstant("xpathRelativeSiteNameMenuLink");
  private static final String XPATH_ABSOLUTE_SIDE_BAR_COLLAPSE_BUTTON = Settings.getByConstant("xpathSideBarCollapseButton");
  ...
  public void clickOnSiteGroup(String groupName) {
    (new WebDriverWait(settings.getDriver(), Duration.ofSeconds(settings.getTimeoutSeconds()))).until(
            ExpectedConditions.presenceOfElementLocated(By.xpath(xpathFormat(XPATH_RELATIVE_SITE_GROUP, groupName))));

    WebElement siteGroup = settings.getDriver().findElement(By.xpath(xpathFormat(XPATH_RELATIVE_SITE_GROUP, groupName)));
    siteGroup.click();
  }

  public void clickOnSitePageInGroup(String sitePage) {
    (new WebDriverWait(settings.getDriver(), Duration.ofSeconds(settings.getTimeoutSeconds()))).until(
            ExpectedConditions.presenceOfElementLocated(By.xpath(xpathFormat(XPATH_RELATIVE_SITE_PAGE_LINK_IN_SITE_GROUP, sitePage))));

    WebElement sitePageElement = settings.getDriver().findElement(By.xpath(xpathFormat(XPATH_RELATIVE_SITE_PAGE_LINK_IN_SITE_GROUP, sitePage)));
  ...
  public void toggleSidebar() {
    if (LOG.isDebugEnabled()) LOG.debug("TOGGLE SIDEBAR");
