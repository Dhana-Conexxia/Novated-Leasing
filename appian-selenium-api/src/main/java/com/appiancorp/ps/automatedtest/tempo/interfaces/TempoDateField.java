...
public class TempoDateField extends AbstractTempoField {
  ...
  private static final boolean isPreVersion213 = Settings.getVersion().compareTo(new Version(21, 2)) <= 0;
  ...
  private void populateTempoDateFieldWithDate(WebElement fieldLayout, Date d) {
    ...
    if (dateField.isDisplayed()) {
      ...
      (new WebDriverWait(settings.getDriver(), Duration.ofSeconds(settings.getTimeoutSeconds()))).until(ExpectedConditions.visibilityOf(dateField));
      dateField.sendKeys(dateValue);
    }

    if (isPreVersion213) {
      // Selects the date and dismisses the calendar popup
