...
public class TempoPickerFieldSelection extends TempoPickerField implements FieldLayoutWaitForReturn {
  ...
  public void waitFor(WebElement fieldLayout, String... params) {
    (new WebDriverWait(settings.getDriver(), Duration.ofSeconds(settings.getTimeoutSeconds()))).until(ExpectedConditions.visibilityOf(fieldLayout.findElement(By.xpath(getXpath(
        fieldLayout, params)))));
  }
  ...
  public boolean waitForReturn(int timeout, WebElement fieldLayout, String... params) {
    try {
      (new WebDriverWait(settings.getDriver(), Duration.ofSeconds(timeout))).until(ExpectedConditions.presenceOfElementLocated(By.xpath(getXpath(fieldLayout,
        params))));
      return true;
