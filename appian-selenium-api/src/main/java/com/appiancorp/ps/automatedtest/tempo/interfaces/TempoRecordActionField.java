...
public class TempoRecordActionField extends AppianObject implements Clickable {
  ...
  private static final String XPATH_ABSOLUTE_RECORD_ACTION_FIELD_INDEX_BUTTON = Settings.getByConstant("xpathAbsoluteRecordActionFieldIndexButton");
  ...
  public void click(String... params) {
    if (LOG.isDebugEnabled()) LOG.debug("GET RECORD ACTION FIELD [" + getParam(0, params) + "]");
    ...
    ((JavascriptExecutor) settings.getDriver()).executeScript("arguments[0].click();", recordActionComponentElement);
    ...
    if (option == null) {
      ...
      if (LOG.isDebugEnabled()) LOG.debug("CLICK ACTION [" + actionValue + "]");
      clickElement(option);
      unfocus();
    }
  }
  ...
  public void waitFor(String... params) {
    ...
    if (LOG.isDebugEnabled()) LOG.debug("WAIT FOR RECORD ACTION FIELD [" + getParam(0, params) + "]");
    try {
  ...
  private WebElement getOption(WebElement selectField, String actionValue, Integer currentIndex) {
    ...
    if (currentIndex != null) {
      ...
      for (Iterator<WebElement> iterator = listItems.iterator(); iterator.hasNext();) {
        ...
        if (next.findElement(By.xpath(".//span[contains(@class, 'MenuItem---primary_text')]")).getText().equals(actionValue)) {
          return next;
        }
      }
    }
