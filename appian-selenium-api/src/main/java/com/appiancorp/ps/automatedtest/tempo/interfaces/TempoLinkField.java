...
public class TempoLinkField extends AbstractTempoField {
  ...
  private static final String XPATH_ABSOLUTE_LINK_FIELD_LABEL_INDEX = "(" + XPATH_ABSOLUTE_LINK_FIELD_LABEL + ")[%2$d]";
  ...
  public String getXpath(String... params) {
    ...
    if (isFieldIndex(fieldName)) {
      ...
      if (StringUtils.isBlank(name)) {
        return xpathFormat(XPATH_ABSOLUTE_LINK_FIELD_INDEX + TempoFieldFactory.XPATH_CONCAT_ANCESTOR_FIELD_LAYOUT, index);
      } else {
        return xpathFormat(XPATH_ABSOLUTE_LINK_FIELD_LABEL_INDEX + TempoFieldFactory.XPATH_CONCAT_ANCESTOR_FIELD_LAYOUT, name,
          index);
      }

    } else {
      return xpathFormat(XPATH_ABSOLUTE_LINK_FIELD_LABEL + TempoFieldFactory.XPATH_CONCAT_ANCESTOR_FIELD_LAYOUT, fieldName);
    }
  }
  ...
  public boolean contains(WebElement fieldLayout, String... params) throws Exception {
    ...
    if (LOG.isDebugEnabled())
      LOG.debug("LINK FIELD COMPARISON : Field value [" + fieldValue + "] compared to Entered value [" + compareString + "]");
  ...
  public String capture(WebElement fieldLayout, String... params) {
    ...
    if (LOG.isDebugEnabled()) LOG.debug("LINK FIELD VALUE : " + value);
