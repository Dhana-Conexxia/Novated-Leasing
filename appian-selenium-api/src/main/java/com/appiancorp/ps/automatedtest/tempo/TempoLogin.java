...
public class TempoLogin extends AppianObject {
  ...
  private static final String XPATH_CUSTOM_LOGIN_SUBMIT_BUTTON_FORMAT = "//*[@id='%1$s' or @name='%1$s' or @value='%1$s' or text()='%1$s']";
  ...
  public void navigateToLoginPage(String url) {
    ...
    try {
      (new WebDriverWait(settings.getDriver(), Duration.ofSeconds(settings.getTimeoutSeconds()))).until(ExpectedConditions.presenceOfElementLocated(By.xpath(XPATH_ABSOLUTE_LOGIN_BOX)));
    } catch (Exception e) {
      try {
        logout();
      } catch (Exception e2) {
