...
public class TempoTagField extends AbstractTempoField implements WaitForReturn {
  ...
  protected static final String XPATH_RELATIVE_TAG_ITEM_INDEX = "(" + Settings.getByConstant("xpathRelativeTagItem") + ")[%2$d]";
  ...
  public boolean waitForTagItemReturn(String tagItem) {
    ...
    try {
      (new WebDriverWait(settings.getDriver(), Duration.ofSeconds(settings.getTimeoutSeconds()))).until(ExpectedConditions.visibilityOfElementLocated(By.xpath(tagItemAbsoluteXpath)));
      return true;
    } catch (TimeoutException e) {
      return false;
    } catch (Exception e) {
  ...
  public void waitFor(String... params) {
    ...
    if (LOG.isDebugEnabled()) LOG.debug("WAIT FOR TAG FIELD [" + fieldName + "]");

    try {
      (new WebDriverWait(settings.getDriver(), Duration.ofSeconds(settings.getTimeoutSeconds()))).until(ExpectedConditions.presenceOfElementLocated(By.xpath(getXpath(params))));
    } catch (Exception e) {
      throw ExceptionBuilder.build(e, settings, "Wait for Tag Field", fieldName);
    }
  }
  ...
  public void waitForAbsoluteTagItem(String tagItem) {
    if (LOG.isDebugEnabled()) LOG.debug("WAIT FOR TAG ITEM [" + tagItem + "]");

    try {
      (new WebDriverWait(settings.getDriver(), Duration.ofSeconds(settings.getTimeoutSeconds()))).until(ExpectedConditions.presenceOfElementLocated(By.xpath(getTagItemAbsoluteXpath(tagItem))));
    } catch (Exception e) {
      throw ExceptionBuilder.build(e, settings, "Wait for Tag Item", tagItem);
    }
  }
