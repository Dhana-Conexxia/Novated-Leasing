...
public class TempoRecordTypeUserFilter extends AppianObject implements Clickable {
  ...
  public void populate(String... params) {
    ...
    try {
      ...
      throw ExceptionBuilder.build(new Exception("Dropdown value not found."), settings, "Record Type User Filter",
  ...
  public void populateDateRange(String... params) {
    ...
    try {
      ...
      (new WebDriverWait(settings.getDriver(), Duration.ofSeconds(settings.getTimeoutSeconds())))
        .until(ExpectedConditions.visibilityOfElementLocated(By.xpath(XPATH_ABSOLUTE_RECORD_TYPE_DATE_RANGE)));

      // Populate Start Date
      WebElement startDateInput = settings.getDriver().findElement(By.xpath(XPATH_ABSOLUTE_RECORD_TYPE_DATE_RANGE_START_DATE));
      startDateInput.click();
      startDateInput.sendKeys(userFilterStartDate);

      // Populate End Date
      WebElement endDateInput = settings.getDriver().findElement(By.xpath(XPATH_ABSOLUTE_RECORD_TYPE_DATE_RANGE_END_DATE));
      endDateInput.click();
  ...
  public void clear(String... params) {
    ...
    try {
      ...
      if (Settings.getVersion().compareTo(new Version(17, 1)) < 0) {
        ...
        clear = settings.getDriver()
            .findElement(By.xpath(xpathFormat(XPATH_ABSOLUTE_RECORD_TYPE_USER_FILTER_CLEAR, userFilterName)));
      }
  ...
  private void clickElementInDropdown(String... params) {
    String userFilter = getParam(0, params);
    List<WebElement> dropdowns = settings.getDriver().findElements(By.xpath(XPATH_ABSOLUTE_RECORD_TYPE_USER_FILTER_DROPDOWN));

    if (dropdowns.isEmpty()) {
      throw ExceptionBuilder.build(new Exception("No filter dropdowns were found."), settings, "User Filter", userFilter);
    }
    ...
    throw ExceptionBuilder.build(new Exception("Searched for value not found in any filter dropdowns."), settings, "User Filter",
      userFilter);
  }
