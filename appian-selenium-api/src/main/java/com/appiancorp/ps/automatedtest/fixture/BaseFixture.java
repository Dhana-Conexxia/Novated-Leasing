...
public class BaseFixture {
   ...
   *
   * @param username
   *          ID or NAME of the username field in the login page
   * @param password
   *          ID or NAME of the password field in the login page
   * @param loginButton
   *          NAME for the login/submit button in the login page (Optional)
   */
  public void setupLoginWithUsernameFieldAndPasswordFieldAndLoginButton(String username, String password, String loginButton) {
  ...
  public void setupWithBrowser(String browser) {
    ...
    if (browser.equals(Constants.Driver.FIREFOX.name())) {
      ...
      if (driverHome != null) System.setProperty("webdriver.gecko.driver", driverHome);
      ...
      if (!StringUtils.isBlank(props.getProperty(Constants.DOWNLOAD_DIRECTORY))) {
        ...
        prof.setPreference("browser.download.dir", FilenameUtils.separatorsToSystem(props.getProperty(Constants.DOWNLOAD_DIRECTORY)));
        prof.setPreference("browser.helperApps.neverAsk.saveToDisk", props.getProperty(Constants.DOWNLOAD_MIME_TYPES));
      }
      FirefoxOptions firefoxOptions = new FirefoxOptions();
      firefoxOptions.setProfile(prof);
      if (!StringUtils.isBlank(props.getProperty(Constants.Driver.FIREFOX.getBrowserHome()))) {
        File pathToBinary = new File(FilenameUtils.separatorsToSystem(props.getProperty(Constants.Driver.FIREFOX.getBrowserHome())));
      ...
      if (driverHome != null) System.setProperty("webdriver.chrome.driver", driverHome);
      ...
      //Chromedriver v2.28 onwards, when "--disable-extensions" flag is passed, it implicitly passes "disable-extensions-except" flag which loads Chrome automation extension, therefore we need to explicitly set "useAutomationExtension" option to be false
      ...
      if (StringUtils.isNotBlank(props.getProperty(Constants.Driver.CHROME.getBrowserHome()))) {
        co.setBinary(FilenameUtils.separatorsToSystem(props.getProperty(Constants.Driver.CHROME.getBrowserHome())));
        co.addArguments("--disable-extensions");
   ...
   * 
   * @param filename
   *          Filename to delete out of the download directory
   */
   ...
   * 
   * @param localePath
   *          Absolute path to the JSON encoded locale file.
   *          Format of file:
   *          {
   *             "dateFormats": [
   ...
   *
   * @param x
   *          X coordinate on the primary monitor
   * @param y
   *          Y coordinate on the primary monitor
   */
   ...
   *          Appian url
   * @param username
   *          Appian username, must match username in users.properties
   */
   ...
   *          Appian url
   * @param role
   *          Role matching role in users.properties
   */
   ...
   * 
   * @param datetime
   *          Datetime string must match yyyy-mm-dd HH:mm
   */
   ...
   * 
   * @param webApiEndpoint
   *          Web api endpoint, not including url
   * @param username
   *          Appian username
   ...
   *          Test variable key
   * @param val
   *          JSON string containing variable data
   */
