...
public class TempoFileUploadField extends AbstractTempoField {
  ...
  public void populate(WebElement fieldLayout, String... params) throws InterruptedException {
    ...
    toggleVisibilityForAppian17_1(fileUpload);
  ...
  public String capture(WebElement fieldLayout, String... params) {
    List<WebElement> webElements = fieldLayout.findElements(By.xpath(xpathFormat(XPATH_RELATIVE_FILE_UPLOAD_FIELD_FILE)));
    ...
    if (LOG.isDebugEnabled()) LOG.debug("FILE UPLOAD FIELD VALUE : " + values);
  ...
  private ExpectedCondition<Boolean> uploadFieldContainsFile(WebElement uploadField, String expectedFileName) {
    return webDriver -> {
      ...
      if (LOG.isDebugEnabled())
        LOG.debug("FILE UPLOAD FIELD COMPARISON : Field value [" + expectedFileName + "] compared to Entered value [" + compareString + "]");
  ...
  public void clear(WebElement fieldLayout, String... params) {
    List<WebElement> removeLinkElements = fieldLayout.findElements(By.xpath(XPATH_RELATIVE_FILE_UPLOAD_FIELD_REMOVE_LINK));
